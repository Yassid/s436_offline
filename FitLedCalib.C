...
void fit_torchcalib(){
  TH1I *hist[6];
  double pedestal_estimate[6], gain_estimate[6] ;
  double ped[6]={};
  double ped_err[6]={};
  double gain[6]={};
  double gain_err[6]={};
  ...
  //---------------------------
  hist[0]=torch_qdc_cor_0__1 ;  pedestal_estimate[0]=146.;  gain_estimate[0]=6.; //1
  hist[1]=torch_qdc_cor_1__2 ;  pedestal_estimate[1]=181.;  gain_estimate[1]=5.; //2
  hist[2]=torch_qdc_cor_2__3 ;  pedestal_estimate[2]=153.;  gain_estimate[2]=7.; //3
  hist[3]=torch_qdc_cor_3__4 ;  pedestal_estimate[3]=118.;  gain_estimate[3]=5.; //4
  hist[4]=torch_qdc_cor_4__5 ;  pedestal_estimate[4]=136.;  gain_estimate[4]=7.; //5
  hist[5]=torch_qdc_cor_5__6 ;  pedestal_estimate[5]=121.;  gain_estimate[5]=10.; //6
  //---------------------------
  ...
  //----- print result -----
  fprintf(stderr,"TORCH_CALIBRATION_TABLE_BEGIN\n");
  for(int i=0; i<6; i++){
	fprintf(stderr,"%d %f %f %f %f\n",i ,ped[i], ped_err[i], gain[i], gain_err[i]);// ped, ped_err, gain, gain_err;
  }
  fprintf(stderr,"TORCH_CALIBRATION_TABLE_END\n"); 
...
void fit_hirac_calib(){
  TH1I *hist[8];
  double pedestal_estimate[8], gain_estimate[8] ;
  double ped[8]={};
  double ped_err[8]={};
  double gain[8]={};
  double gain_err[8]={};
  TCanvas *c0=new TCanvas("c0","c0",100,0,1200,750);
  c0->Divide(4,2);
		
  //---------------------------
  hist[0]=  newVMEv792q10_00_02__3 ;  pedestal_estimate[0]=1300.;  gain_estimate[0]=100.; //1
  hist[1]=  newVMEv792q10_00_04__4;  pedestal_estimate[1]=1010.;  gain_estimate[1]=240.; //2
  hist[2]=  newVMEv792q10_00_06__5;  pedestal_estimate[2]=920.;  gain_estimate[2]=250.; //2
  hist[3]=  newVMEv792q10_00_08__6;  pedestal_estimate[3]=820.;  gain_estimate[3]=150.; //2
  hist[4]=  newVMEv792q10_01_02__7;  pedestal_estimate[4]=950.;  gain_estimate[4]=200.; //2
  hist[5]=  newVMEv792q10_01_04__8;  pedestal_estimate[5]=979.;  gain_estimate[5]=230.; //2
  hist[6]=  newVMEv792q10_01_06__9;  pedestal_estimate[6]=830.;  gain_estimate[6]=300.; //2
  hist[7]=  newVMEv792q10_01_08__10;  pedestal_estimate[7]=970.;  gain_estimate[7]=120.; //2
  //---------------------------
  ...
  //----- print result -----
  fprintf(stderr,"HIRAC_CALIBRATION_TABLE_BEGIN\n");
  for(int i=0; i<8; i++){
	fprintf(stderr,"%d %f %f %f %f\n",i ,ped[i], ped_err[i], gain[i], gain_err[i]);// ped, ped_err, gain, gain_err;
  }
  fprintf(stderr,"HIRAC_CALIBRATION_TABLE_END\n"); 
...
void fit_spectrum(TH1I *h1, double ped_ini, double gain_ini, double* result_out){
  ...
  fitFcn->SetParLimits(1, ped_ini-60.0,ped_ini+60.0);
  fitFcn->SetParLimits(2, 0.1,200.0);
  fitFcn->SetNpx(2000);
  ...
  fitFcn->SetParLimits(5,1.0,500.0);
  fitFcn->SetParLimits(6,10.0,500.0);
